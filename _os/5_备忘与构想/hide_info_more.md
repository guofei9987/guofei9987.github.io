...
- 方程大概率是无解的
    ...
    - 方案2：作为近似方法，计算 a 的时候，使用 A 和 B 对应的 gray 值。得到的 a 之后，计算 P


我们采用方案2
1. 输入 图片 A，B，其 RGB 通道记为 $A = [A_R, A_G, A_B], B = [B_R, B_G, B_B]$
2. 计算其对应的灰度值，记为 $A_{gray}, B_{gray}$ （可以使用平均值，或标准灰度转换）
3. 计算 `alpha = 255 - (A_gray - B_gray)`，然后 clip 保证其值范围在 0-255
4. 计算结果图片的 RBG 通道，`[R, G, B] = (A-(255-alpha))/(alpha/255)` 就是最后结果
...
2. 在纯黑背景下显示为 $\dfrac{\alpha}{255}RGB+\dfrac{255-\alpha}{255} 0$
    ...
    = $A-A_{gray}+B_{gray} \approx B_{gray}$
3. 以上是理论情况，实际情况是，参与运算的各个值都是 uint8 类型，这引入两个问题：
    1. 截断问题，每个值在最终运算后，都必须落在 `[0,255]`，超出则截断；
    2. 取整问题，每个值最终都必须取整，这在某些情况下放大了误差，使最终结果不如理论值完美。


**偏差分析**
1. 如果某个像素点 $B > A$，那么 $\alpha>255$，clip 到 255，RGB 值与图片 A 完全一致。也就是说，这个情况下，B 像素值不在结果中体现
2. 如果 B 特别小呢？
2. 尤其是 $A_{gray} - B_{gray}$ 接近 255 时，$\alpha$ 接近 0，它作为分母取整会导致值剧烈变化。
    - 最极端的情况，如果 `A=255,B=0`，那么 `alpha = 0`；之后 `alpha` 作为除数，导致 P 值为无穷大，clip 到 255
    - 后果就是：合成图片在白色背景下，还是能隐约看到图片 B 的一些轮廓（是吗？）
    - 即使 `alpha` 不是0，而是很小的数，
5. 为了解决以上问题，可以先把图片 B 的范围做个映射，使 B 的值不要太小（为了规避取整带来的结果波动问题），同时使 B 值不要太大（为了规避 B>A 时 B 不再显示的问题），使用线性映射 `B = a * B + b`，
    ...
    - 调整两个参数有些费劲，为了减少一个参数，可以添加限制 `a * 255 + b < img1_avg`，从而令 `b = img1.mean() - 255 * a` （未必是最优的，最优应当做像素级限制）
