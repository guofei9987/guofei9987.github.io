...
3. 以上是理论情况，实际情况是，参与运算的各个值都是 uint8 类型，这导致2个后果：1）截断问题，每个值在最终运算后，都必须落在 `[0,255]`，超出则截断；2）取整问题，每个值最终都必须取整，这在某些情况下放大了误差，使最终结果不如理论值完美。

**偏差的具体表现与解决方案**
1. 尤其是 $A_{gray} - B_{gray}$ 接近 255 时，$\alpha$ 接近 0，它作为分母取整会导致值剧烈变化。
    - 最极端的情况，如果 `A=255,B=0`，那么 `alpha = 0`；之后 `alpha` 之后作为除数，导致 P 值为无穷大，clip 到 255
    - 后果就是：合成图片在白色背景下，还是能隐约看到图片 B 的一些轮廓（是吗？）
    - 即使 `alpha` 不是0，而是很小的数，
4. 如果某个位置的像素点 B 值 比 A 值大，那么 $\alpha>255$，clip 到 255，RGB 值与图片 A 完全一致，也就是说，B 像素点不在结果中体现
