...
n=data.shape[0]
mac=np.array([True]*int(0.8 * n)+[False]*(n-int(0.8*n)))#掩码，用于随机打乱顺序
shuffle(mac)  # 随机打乱数据
data_train = data[mac]  # 选取80%为训练数据
data_test = data[~mac]  # 选取20%为测试数据

x_train = data_train.loc[:,['x1','x2']]* 30
y_train = data_train.loc[:, 'y']#svm的y需要输入1darray
x_test = data_test.loc[:,['x1','x2']]* 30
