...
Probit模型与Logit模型没有本质区别，模型结果的差别也不大。区别是Logit模型假设随机变量服从logistics distribution，Probit模型假设随机变量服从正态分布  
...
$Y=k \Longleftrightarrow Y^* \in [u_{k-1},u_k]$  
$\varepsilon$服从标准正态分布  
(因此probit模型适用于：y是有序离散变量的情景)  

## 模型输出
$P(Y=k \mid X)=P(Y^* \in [u_{k-1},u_k] \mid X)=\Phi(u_k-wx)-(u_{k-1}-wx)$
