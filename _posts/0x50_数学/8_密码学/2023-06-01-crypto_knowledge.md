...
| 类型                            | 核心思想               | 示例                     |
| ----------------------------- | ------------------ | ---------------------- |
| **替换密码 Substitution Cipher**  | 用其他符号替代明文中的符号，顺序不变 | 凯撒密码 Caesar Cipher     |
| **置换密码 Transposition Cipher** | 保留原符号，但重新排列顺序      | 栅栏密码 Rail Fence Cipher |
| **积性密码 Product Cipher**       | 结合替代与置换，以增强安全性     | DES、AES（现代密码多属此类）      |
...
**密码安全**
- **无条件安全**：在攻击者拥有无限算力的情况下，一个密码体制也是安全的，叫做 无条件安全。密码学不研究这个。
...
- 多对一：分组，一次加密一组。
    ...
    - 攻击者的词表就是 $26^8$ 大小
    - 字母频率也不明显了
- 一对多（维吉尼亚），明文中一个字母，映射为多个字母


**playfair cipher**（上面的多对一），具体来说，用2个字母映射2个字母，
- 19世纪发明的，最初为了配合电报机使用。
...
- 分组。每两个分一组。
    ...
    - 如果为奇数个，则填充（padding）`X`，这是现代密码的常见方法
