...
RTF | 含义 |
|--|--|
Role |	**指定大模型所扮演的角色身份**，例如程序员、数据分析师、讲解员或记者等角色 <br>目的：限定领域，提升回答准确度，尤其是指令不够清晰时显著有用
...
这部分的视角：搭建 agent 以完成业务需求。

Prompt Engineer vs SFT（为什么需要？）
- 成本（时间成本、GPU成本） 极少或没有
- 快速更新，只需要更新文本，而不是重新训练
- 快速反馈，可以快速调整
- 极少（甚至不需要）数据
- 防止 SFT 的灾难性遗忘
- 举例：基座模型训练后，又上映了新的电影，或者模型的任务是需要生成某个方言的SQL，SFT 代价巨大，且有灾难性遗忘的风险。
- 缺点：有上限，Prompt 越来越复杂，可能出现样例之间冲突的情况。


Prompt 分为3个等级
- System：定义助手的角色、风格、行为边界，整场对话持续生效，优先级高于用户输入。
- User：用户提供的指令
- assistant：模型生成的消息


**关注的参数**：
- context window：最大 token数量，现在（写此文章时）很多模型的 context window 是 256K；另外，即使有些模型能达到 1M 以上，其前后文也有遗忘的情况。
- temperature：控制随机性的东西。
    - **主流模型都不建议设置为0**，原因是限制了模型在文本空间的思考
    - 即使设置为0，每次输出也不是固定的
    - DeepSeek V3 官方建议 temperature=0.3，R1 为 0.5～0.7
    - Qwen3 思考模式官方建议 temperature=0.6，非思考模式为 0.7
- top P，0～1的数字，表示从前 N% 找 next Token
- top K，整数，表示从前 K 个候选集中找 next Token



有效 Prompt 的原则：规避“知识的诅咒”。
- 你与同事讨论一个任务时，实际上忽略了大量的“共同知识”，因此往往一两句话就能说清楚
- 但是 LLM 是不知道这些“共同知识”的
- 因此你需要仔细检查，并明确写出所有的“共同知识”，把 LLM 当成聪明但特定领域一无所知的人。


prompt 自动化迭代
- Google 的一篇文章：https://research.google/blog/making-complex-text-understandable-minimally-lossy-text-simplification-with-gemini/
- 目标是提取网页中的主要内容
- 方法是自动化迭代，用 LLM 来评估另一个 LLM 的 输出，并根据评估指标来自动优化 prompt 
- 每轮自动化评估：可读性、忠实度
- 824 次迭代后，趋于稳定
...
防止幻觉
1. `请严格根据检索结果给出结论`，`禁止编造事实`，`给出的资料附上链接`
2. 允许大模型无法回答，`如果根据上下文无法回答，请回复“我没有足够的信息来回答这个问题”`
...
- 很快，收益开始递减
    - 修复一个边缘案例，会引发另一个问题
    - 小的调整会带来不可与测的倒退
    - 总有些目标，无论如何更改提示词都无法达到
- 这是到达了 Prompt 工程的天花板
- 这就需要 Context Engineer
...
Jinja 是一种 Python 模版引擎，最初是在 Web 领域使用，现在又在 LLM 领域发扬光大了。

可以用类似 `【% if xxx %】` 之类的，选择性的给每次任务不同的 Prompt 


### 结构化输出

很多时候，需要模型输出结构化数据（例如 JSON）


JSON Schema / Structured Output
- 原理：在模型输出 Token 时，做硬性约束，使格式不合适的 token 不会被选到
- 核心机制：Constrained Decoding
    - 它会在输出每个 token 时判断，当前哪些 token 能让输出符合 Json，然后把那些不符合 Json 的 token 概率置为零。
- 常见问题：模型还“聪明”吗？还聪明
    - 限制的是 形式空间（syntax & schema）
    - 可以理解为：给模型一个“只能填表格”的环境，但每一格填什么，仍由模型决定。



下面给个例子：

![:caption Structured Output](/a/nn/structured_output.svg)
...
```json
{
  ...
  "properties": {
    ...
    "pop_m": {
      "type": "number",
      "minimum": 0,
      "description": "Population in millions (approximate)"
    },
    "coordinates": {
      "type": "object",
      "additionalProperties": false,
  ...
  "required": ["name", "country", "pop_m", "coordinates"]
}
```
...
- 最重要的指令，放后面更好。因为 LLM 的 Next Token 特点



### 长记忆

多个 LLM 协助时，为了让它们共享一部分信息，但又不接受过多的信息，可以设计一套“虚拟文件系统”，下面是一套例子：
- `write_context(name,text,mode)`： 创建/覆盖/增量更新已有的文件
- `read_context(name)`: 读入指定的文件
- `list_context()`: 列出所有文件
