...
HDR（High Dynamic Range，高动态范围），比传统的 SDR（Standard Dynamic Range，标准动态范围）更能呈现人眼实际看到的亮度和层次。
- 本质上是在亮度范围（dynamic range）上做扩展的一套技术体系，  
- 核心目标是：同时保留“很亮”和“很暗”的细节。
...
应用场景：
- 视频、照片、电影：提升明显
- 游戏：高光（爆炸、阳光）更真实，阴影区域更可见
- 收集拍照：“多帧合成HDR”（自动融合多张不同曝光照片）

-------

底层原理

使用 **伽马** 的原因：人眼感知是非线性的，较暗区间精度高（平方根函数）、较亮区域精度低（对数函数）。因此需要用动态精度。



HDR标准出版物使用BT2020不仅是为了更大的色域范围，也是为了完全兼容以前的“旧色域”
...
**ICC** profile（ICC 色彩配置文件） 是一份用来描述“某个设备或文件里的颜色应该怎么理解、怎么转换”的标准化色彩描述文件
...
| 名称 | 主要用途 | 典型类型 | 更像什么 |
|---|---|---:|---|
| ICC profile | 颜色管理 | 色彩描述文件 | 颜色翻译规则 |
| EXIF | 拍摄信息 | 图像元数据 | 拍摄记录 |
| XMP | 扩展信息 | 通用元数据 | 附加标签/说明 |
| HDR10 metadata | HDR 显示提示 | 静态 HDR 元数据 | 播放参考说明 |
