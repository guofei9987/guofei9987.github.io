...
**DWT2（2D 离散小波变换）**：
1. **基本概念**：2D DWT 是在二维数据（如图像）上应用 1D DWT，首先对图像的每一行进行 1D DWT，再对每一列进行 1D DWT。
...
3. **DWT2 应用**：在图像处理中，DWT2 主要用于图像压缩、去噪和水印技术。图像通过 DWT2 变换后，可以有效地提取和处理图像的多层次信息。
...
$$
I_{LL} = DWT(I)_{\text{low,low}} , \quad I_{LH} = DWT(I)_{\text{low,high}}
$$

$$
I_{HL} = DWT(I)_{\text{high,low}}, \quad I_{HH} = DWT(I)_{\text{high,high}}
$$
