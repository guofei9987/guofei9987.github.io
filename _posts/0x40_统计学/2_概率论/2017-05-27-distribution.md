...
|二项分布Binomial distribution|$X \sim b(n,p)$|$P(X=k)=\dbinom{n}{k}p^k(1-p)^{n-k}$|$EX=np$<br>$DX=np(1-p)$|可加性：$b(n_1,p)+b(n_2,p)=b(n_1+n_2,p)$|0-1分布是一种特殊的二项分布<br>二项分布是n个独立同分布的0-1分布的加和|
