...
class Solution:
    def solveSudoku(self, board: List[List[str]]) -> None:
        ...
        """
        workable = [[None for i in range(9)] for j in range(9)]
            ...
            for j in range(9):
                if board[i][j] == '.':
                    tmp = all_nums \
                               ...
                               - {board[i // 3 * 3 + i1][j // 3*3 + j1] for i1 in range(3) for j1 in range(3)}

                    workable[i][j]=tmp

                    blank_idx.append([i,j])

        def find_res(workable,idx,res):
            i,j=blank_idx[idx]
            for choose in workable[i][j]:
                workable_=workable.copy()
                res[idx]=choose
                workable_[i][j]=
```
