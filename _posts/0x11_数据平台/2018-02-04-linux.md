...
功能相似，各有侧重
- curl 适合：调API、发送 GET/POST/PUT/DELETE、发送 header
- wget 适合：下载文件、断点续传


curl
```shell
curl https://www.guofei.site/
...
# -i 把响应头也输出出来
curl -i https://www.guofei.site/
# -I 只看响应头
curl -I https://www.guofei.site/


# -L 自动跟随重定向（301 Moved Permanently, 302 Found）
curl -L http://example.com

# 静默模式
 curl -s https://www.guofei.site/
# -s：不显示进度条和错误信息
curl -sS https://www.guofei.site/
# -sS: 静默，但仍显示错误

curl --verbose https://www.guofei.site/

# 指定建立链接超时时间、整体超时时间
curl --connect-timeout 5 --max-time 20 https://www.guofei.site/
...
```sh
命令1 | 命令2 | 命令3
# 解释：命令1 的输出传给 命令2，命令2 的输出再传给 命令3
...
# 举例2:
echo $PATH | cut -d ':' -f 3
# 功能：显示环境变量 PATH；将其按 : 分割，并取出第 3 个。（cut 的介绍在上面）
