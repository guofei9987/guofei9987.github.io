...
json_tuple：可以同时提取多个字段，仅用于类型1，需要配合 LATERAL VIEW 中使用：
```sql
...
如何展开 Array 中的每个 item 为一行？不同的数据库有不同的方法：1）可以用正则分割，2）用 UDF，3）某些数据库可以这样：
...
LATERAL VIEW EXPLODE(
    -- ARRAY<STRING>
    FROM_JSON(json_str, 'ARRAY<STRING>')
) t AS item;
```
...
-- 生成 JSON
CREATE TABLE tmp_example_json4 AS
SELECT  TO_JSON(MAP("name", name,"sex", sex,"age", age)) AS json_data
...
SELECT  json_data,
FROM_JSON(json_data, 'MAP<string,string>') AS json_data_map -- 把 json_data(STRING类型) 解析成 MAP 类型
-- 根据 json_data 不同的结构，填入的 schema 信息 还可以是 ARRAY<string> 等类型

-- 因此，可以这样（覆盖式）增加信息：
,TO_JSON(
    MAP_CONCAT(FROM_JSON(json_data, 'MAP<string,string>'), MAP('class', '三年二班'))
    ) 
AS json_data_new
FROM tmp_example_json4;



-- 如果 json_data 的 schema 是 ARRAY<string>，还可以搭配 LATERAL VIEW 来展开它
...
```sql
CREATE TABLE tmp_example_item AS
SELECT group_id, item, cnt FROM 
(
    VALUES
    ('A','apple',3),
    ('A','apple',5),
...
SELECT
    group_id,
    HISTOGRAM(item) AS item_map
FROM tmp_example_item
GROUP BY group_id;
-- 输出 MAP<STRING, INT>, 意义是每个 item 的数量

SELECT
    group_id,
    MAP_UNION_SUM(MAP(item,cnt)) AS item_map
FROM tmp_example_item
GROUP BY group_id;
-- 输出 MAP<STRING, INT>, 意义是每个 item 的 cnt 之和

SELECT
    group_id,
    MAP_UNION(MAP(item,cnt)) AS item_map
FROM tmp_example_item
GROUP BY group_id;
-- 输出 MAP<STRING, INT>, 意义是每个 item 的 cnt，选取其一保留
```
