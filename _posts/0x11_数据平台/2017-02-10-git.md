...
是什么？
- 一种方案，使项目目录和 .git 对象文件可以分离
- 适合：
    - 管理 ~/.zshrc
    - 管理配置文件
    - 存放在 iCloud 上的笔记项目，同时备份到 GitHub（这里的示例）
...
初始化
```sh
git init --bare ~/.plans-git

alias plan='git --git-dir=$HOME/.plans-git --work-tree=$HOME/Library/Mobile\ Documents/com~apple~CloudDocs/Obsidian/plans'
```
