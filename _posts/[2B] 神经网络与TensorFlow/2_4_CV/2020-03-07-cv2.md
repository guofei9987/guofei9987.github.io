...
cv2.approxPolyDP()采用的是 Douglas-Peucker 算法(DP 算法)。以图 12-23 为例，该 算法首先从轮廓中找到距离最远的两个点，并将两点相连(见图 12-23 的(b)图)。接下来，在 轮廓上找到一个离当前直线最远的点，并将该点与原有直线连成一个封闭的多边形，此时得到 一个三角形。  
