...
做法
- 对于规则性强的命名实体，例如网址、email、ISBN、商品编号。可以用正则，未能正则的片段交给统计模型
- 对于较短的命名实体，可以用分词算法确定边界，然后用词性标注算法确定类别。往往足以解决问题。
- 对于复合词命名识别的任务，可以转化为序列标注问题，转为“BMES”标签，对于命名实体之外的词，标为 “O” 标签
...
- 词典中的汉字，可能是常用词，这会造成大量误召回，所以词典改成双词。
