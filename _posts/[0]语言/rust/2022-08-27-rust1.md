...
let res: Result<i32, String> = divide(10, 0);
...
```Rust
// 一个返回 Result的函数
fn my_func(inp: i32) -> Result<i32, String> {
    if inp > 5 { Ok(inp + 1) } else { Err(format!("{}", inp - 1)) }
}

let res = my_func(6);
match res {
    Ok(T) => { println!("ok with {:?}", T) }
...
如何解Result
```Rust
// 1. unwrap 遇到错误直接panic
let f = File::open("a.txt").unwrap();
// 2. 如果错误后，想自定义 panic 内容
let f = File::open("a.txt").expect("出错啦！");
// 3. 可以更自由的方法
let f = File::open("a.txt").unwrap_or_else(|error| panic!("出错啦! {:?}", error));

// 4. Error有类型
let f = File::open("a.txt").unwrap_or_else(
    |error| {
        if error.kind() == ErrorKind::NotFound { panic!("没找到文件{}", error) }
        else { panic!("其它错误 {}", error) }
    });
// 5. 最简洁的形式。链式处理 Result，同时还能把错误传递下去
