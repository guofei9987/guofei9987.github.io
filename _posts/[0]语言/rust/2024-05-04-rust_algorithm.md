...
知识点
1. `#[inline]` 内连优化，在使用这个函数的地方，直接替换为函数内部的代码本身
    - 优点：提高性能
    - 缺点：编译时代码膨胀。
2. `Box` 允许在堆上分配内存，允许递归结构，从而允许递归定义的大小在编译时是未知的
3. `PartialEq` 允许 `!=` 和 `==`，`Eq` 是 `PartialEq` 的子 trait，相等性比较是完全的
4. `Clone` 允许显式深复制，也就是 `a.clone()`，`Copy`允许 `隐式浅复制`，也就是需要时自动复制。
