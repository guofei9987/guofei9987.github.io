...
Dual from（对偶问题）
$\max Q(\alpha)=\sum\limits_{i=1}^l \alpha_i -0.5\sum_{i=1}^l\sum_{j=1}^l\alpha_i\alpha_j y_i y_j (x_i \cdot x_j)$  
s.t. $\sum\limits_{i=1}^l \alpha_iy_i=0,\alpha_i \geq 0$


Decision function
$f(x)=sgn(\sum\limits_{i=1}^l \alpha^* y_i (x\cdot x)+b^* )$  
$w^* = \sum\limits_{i=1}^l \alpha_i^* x_i y_i$  
