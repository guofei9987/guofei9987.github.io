...
- 有 `lib.rs` 时，项目为 **库**（library crate），不能用 `cargo run`，只能 `cargo build` 和 `cargo test`
...
description = "一段描述"
repository = "https://github.com/guofei9987/hidden_watermark"
license = "MIT"
keywords = ["watermark", "image", "steganography"]

[dependencies]
# std 会隐含的引入，它包含 Vec,Result 等常用的东西
...
# 打包时规定包含和排除。
# 一般不用特意配置，用默认的就可以，如果想手动配置：
# exclude 和 include 一般不同时出现
[package]
# 排除某些目录：
exclude = ["tests/*"]
# 必须加入某些目录：
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("用法如下:");
        eprintln!("写入: {} write <学号> <姓名> <年龄>", args[0]);
        eprintln!("读取: {} read <学号>", args[0]);
        process::exit(1);
    }
    ...
    if mode == "write" {
        ...
        let name = args[3].parse::<String>().unwrap();
        let age = args[4].parse::<i32>().unwrap();
        print!("成功写入，学号={}, 姓名={}, 年龄={} \n", num, name, age);
    } else if mode == "read" {
        let num = args[2].parse::<i32>().unwrap();
        print!("成功读取，学号={}", num);
...
```sh
# 测试
cargo run -- write 1001 bob 18

# 编译并输出一个可执行文件
cargo build --release
# 执行这个可执行文件
my_project write 1001 bob 18
```

如果要使用类似 `--output` 这种入参方式，可以使用 `clap`
...
优点：分发前编译一次，用户直接下载并运行。

几种方案
方案1:GitHub Release，使用 GitHub Actions 来自动化交叉编译。  
用户体验：
```shell
wget xxx
chmod +x
  ...
  build:
    ...
    steps:
      ...
      - name: Package
        ...
        run: |
          ...
          if [ "${{ matrix.archive }}" = "tar.gz" ]; then
            tar -czf \
              ${APP_NAME}-${VERSION}-${{ matrix.target }}.tar.gz \
              -C dist .
          else
      ...
      - uses: softprops/action-gh-release@v2
        with:
          files: |
            ${{ env.APP_NAME }}-${{ env.VERSION }}-${{ matrix.target }}.${{ matrix.archive }}
```
...
其它方案
- 方案2:brew，对项目的用户量、稳定性、质量要求高
    - 可以用 guofei9987/tap，自己维护，门槛低一些
- 方案3:apt install 之类的，十分麻烦，因为 Linux 发行版碎片化。
