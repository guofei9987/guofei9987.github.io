...
# 从 crates.io 下载一个可执行包，编译，然后把它安装到 Cargo bin 目录中
...
// 1.1 如果同一个函数有多种错误类型，可以统一为 Box<dyn std::error::Error>，适合简单项目，频繁错误时有开销
fn my_main() -> Result<(), Box<dyn Error>> {
    let bytes = fs::read("不存在的文件")?;
    let y = func1(-5)?;
    Ok(())
}
...
// 1.2 也可以把它们都转为 Result<T, String> 类型，做到统一。
// （性能差，不推荐）
...
// 1.3 也可以自定义 struct Error
// 这是性能最高的
// （代码略）


// 1.4 可以使用 anyhow，可以方便的传递各种错误
// 有很小的性能损失
