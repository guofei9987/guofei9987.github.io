...
while True:
    ...
    # 这个 conn 是一个新 socket；sk 是监听用，conn 是连接用
...
# 处理信息，并回复，这里是个最简单的 demo
def msg_process(msg):
    msg = msg.decode('utf-8')
    print("收到信息：", msg)
    msg_to_send = msg.replace('吗', '').replace('?', '!').replace('？', '！')
    print("发送信息：", msg_to_send)
    return msg_to_send.encode('utf-8')
...
在 Linux 服务器上
- `127.0.0.1` 是回环地址，它无法被别的机器访问
    - localhost 等价于 `127.0.0.1` 或者 IPv6 的 `::1`
- `0.0.0.0` 可以被别的机器访问
    - 前提：防火墙允许，网络策略允许，网络可达，没有代理、网关、端口映射问题
    - 但是。实际多用 `127.0.0.1` + Nginx 减少暴露端口
- 假设其启动的服务地址为 `0.0.0.0:8080`，服务器 IP 是 `10.20.30.40`
    - 启动的服务地址也可以是 `10.20.30.40:8080`，但通常不这样直接指定 IP

客户机：
- 输入 `http://10.20.30.40:8080` 即可访问
- 客户端也会分配一个Lins的端口，例如 `10.20.50.60:53124`
- 如果服务器IP对应 url，还可以用 url访问
- http 对应 80 端口，https 对应 443 端口时，端口号可以省略


如果服务器需要启动多个网络服务，怎么做？
- 若干个应用分别使用不同的端口：`127.0.0.1:8080`, `127.0.0.1:8081`, `127.0.0.1:9000`
- 用 Nginx 反向代理，统一监听 80 或 443 -> `0.0.0.0:443`
- 用户访问：
    - `https://admin.guofei.site` -> `127.0.0.1:8080`
    - `https://blog.guofei.site` -> `127.0.0.1:8081`
    - `https://api.guofei.site` -> `127.0.0.1:9000`
