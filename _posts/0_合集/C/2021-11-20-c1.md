...
typedef // 声明类型别名
sizeof // 类型占用的字节数，返回 size_t 类型（这个类型在32位系统实际上是 unsigned int，64位系统实际上是 unsigned long）
```
...
// 1. break 必须带，不然的话，匹配成功一个，下面的每个 case 都会执行。因为 C 语言的 switch 用了类似 goto 的设计，这样可以避免多余的判断。
// 2. case 可合并，如下：
...
// 3. 语法上，index 可以溢出，会发生不可预知的行为，例如
a[100] = 999;
a[101];
