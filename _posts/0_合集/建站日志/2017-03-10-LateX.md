...
竖线
- mid $\mid$，常用于集合定义、条件概率
- `\lvert x \rvert`，$\lvert x \rvert$，用于绝对值
- `lVert x \rVert`，$\lVert x \rVert$，常用来表示范数
- `\vert`, $\vert$ 更通用，一般用于矩阵或条件概率
...
|$\xrightarrow{hello}$| xrightarrow | $\xLeftrightarrow{qeual}$ | xLeftrightarrow
...
1. 单 `$` 符号框起来的行内 latex，其大括号容易被吞掉，要用 `$$`
2. 也可以用 $\lbrace / \rbrace$ 来表示
