...
- $D_1 \times D_2 \times ... \times D_n =\left\{ (d_1,d_2,...,d_n)\right\} \mid d_i \in D_i, i=1,...,n$
...
并操作的定义：R和S是相容的，那么 $R \cup S = \left\{ t \mid t\in R \lor t \in S\right\}$
...
定义：前提是 R 和 S 是相容的，那么 $R - S = \left\{ t \mid t\in R \lor t \not\in S\right\}$
...
用数学语言定义：$R \times S = \left\{ (a_1, a_2,..., a_n, b_1, b_2,..., b_n) \mid (a_1, a_2,..., a_n) \in R \land (b_1, b_2,..., b_n) \in S\right\}$
...
$\sigma_{con} (R) = \left\{ t \mid t \in R \land \mathrm{con}(t) = \mathrm{true} \right\}$
...
数学描述: 假设 $R(A_1, A_2, ..., A_n)$，那么 $\pi_{A_{i_1},A_{i_2},...A_{i_k}} = \left\{(t[A_{i_1}], t[A_{i_2}],..., t[A_{i_k}]) \mid t \in R\right\}$
- 其中：
- $\left\{A_{i_1},A_{i_2},...A_{i_k}\right\} \subseteq \left\{ A_1, A_2, ... , A_n\right\}$
...
定义：前提是 R 和 S 是相容的，那么 $R \cap S = \left\{ t \mid t\in R \land t \in S\right\}$
