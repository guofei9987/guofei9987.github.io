...
- 幻读
    - 例子: 计算平均工资时，先计算 SUM(A)，再计算 COUNT(A)，如果另一个事务在中间 UPDATE 加入一条数据，那么计算的值不对应任何时刻的真实值
...
- 读-写、写-读并发：
    - 若 T 读 x，则比较 `TS` 与 `WT(x)`
        ...
        - 否则，有冲突，撤回，重启 T
    - 若 T 写 x，则比较 `TS(T)` 与 `RT(x)`
        - 若 `TS(T) > RT(x)`，则允许操作，并更新 `WT(x) = max(WT(x), TS)`
        - 否则，有冲突，撤回，重启 T
- 写-写
    - 若 T 写 x，则比较 `TS` 与 `WT(x)`
    - 若 `TS > WT(x)`，则允许操作，并更新 `WT(x) = TS`
    - 否则，有冲突，撤回，重启 T
