...
![caption: 数据库系统](/a/computer/database/overview.svg)
...
![逻辑查询优化](/a/computer/database/opt_logical.svg)
...
![caption: 并发中的一些不一致现象](/a/computer/database/transaction_1.svg)
...
![caption: 冲突可串行/非冲突可串行](/a/computer/database/transaction_2.svg)
...
![caption: 用锁](/a/computer/database/transaction_lock.svg)
...
![caption: 相容性矩阵](/a/computer/database/transaction_lock2.svg)
...
![caption: 封锁协议加锁时机](/a/computer/database/transaction_lock3.svg)
...
![caption: 两段封锁协议](/a/computer/database/transaction_lock4.svg)
...
- 对数据表的每个元素，保留最大的时间戳
    ...
    - `WT(x)` 写 x 的最大时间戳
- 事务 T 的时间戳 `TS`
- 若 T 读 x，则比较 `TS` 与 `WT(x)`
    - 若  `TS >= WT(x)`，则允许操作，并更新 `RT(x) = max(RT(x), TS)`
    - 否则，有冲突，回滚，重启 T
- 若 T 写 x，则比较 `TS` 与 `RT(x)`，并且比较 `TS` 与 `WT(x)`
    - 若 `TS >= RT(x) and TS >= WT(x)`，则允许操作，并更新 `WT(x) = TS`
        - 解释：比较 `RT` 以规避 写-读冲突，比较 `WT` 以规避 写-写 冲突
    - 否则，有冲突，回滚，重启 T，并且给 T 一个更大的时间戳



![caption: 基于时间戳的调度算法1](/a/computer/database/transaction_timestamp1.svg)
...
- 调度器维护三个集合：
    ...
    - **VAL集合**。已经确认有效性但尚未完成第3阶段写的事务。对此集合中的事务，调度器维护 `START(T)` 和 `VAL(T)`（T确认的时间）。
    - **FIN集合**。已经完成第3阶段的事务。对这样的事务T, 调度器记录 `START(T)`, `VAL(T)` 和 `FIN(T)`（T完成的时间）。
...
![caption: 基于有效性确认的冲突规则](/a/computer/database/validation_fails.svg)
...
![caption: 故障时间序列](/a/computer/database/log.svg)
...
![caption: 读写性能和日志策略](/a/computer/database/logging_scheme.svg)
...
- 对于动态检查点，查找最后一个 `<END CKPT>` 从对应 `<START CKPT>` 开始 Redo
...
哈尔滨工业大学：数据库系统[【上】：模型与语言](https://www.icourse163.org/course/HIT-1001516002?tid=1475425441)、[【中】：建模与设计](https://www.icourse163.org/course/HIT-1001554030?tid=1475425442)、[【下】：管理与技术](https://www.icourse163.org/course/HIT-1001578001)
