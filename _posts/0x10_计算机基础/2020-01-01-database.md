...
- 依赖的否定，记做 $X \not \to Y$
- $X to Y$ 有两种理解：1）基于模式 R，则要求任意可能的 r 都成立。2）基于某个具体的关系 r，对其成立即可。
...
部分函数依赖、完全函数依赖
:    $R(U)$ 中，如果 $X \to Y$，并且对于 $X$ 上任意真子集 $X'$ 都有 $X' \to Y$ 则称为 $Y$ **完全函数依赖** 于 $X$，记做 $X \xrightarrow{f} Y$，否则称为 $Y$ **部分函数依赖** 于 $X$，记做 $X \xrightarrow{p} Y$
