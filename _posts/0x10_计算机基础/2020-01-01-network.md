...
**IP分片**（IP fragmentation）：一个 IP 数据报太大，超过了下一段链路允许承载的最大大小（**MTU**，Maximum Transmission Unit，最大传输单元），于是被拆成多个更小的 IP 数据报分别传输。
- 分片后的每个 fragment 自己都是一个完整的 IP datagram，都有自己的 IP header
- 它们各自经过网络
