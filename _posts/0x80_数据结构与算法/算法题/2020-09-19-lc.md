...
c版本
```c
bool isMatch1(const char *s, const char *p, const int len_s, const int len_p, bool **dp, int i, int j) {

    if (j >= len_p) {
        return i >= len_s;
    }

    int first_match = (i < len_s) && (p[j] == s[i] || p[j] == '.');
    if (len_p - j >= 2 && p[j + 1] == '*') {

        return isMatch1(s, p, len_s, len_p, dp, i, j + 2)
               || first_match
                  && isMatch1(s, p, len_s, len_p, dp, i + 1, j);
    } else {

        return first_match && isMatch1(s, p, len_s, len_p, dp, i + 1, j + 1);
    }
...
bool isMatch(char *s, char *p) {
    ...
    return isMatch1(s, p, strlen(s), strlen(p), dp, 0, 0);
}
