...
$l_p$空间与$L_p$空间不同，这里研究$l_p$空间  

Def：$l_p$空间的定义  
$$l_P=\{  x=\{x_j\}_{j=1}^\infty \mid \space \mid \mid x \mid \mid_p<\infty \}$$  

这里的$l_p$是一系列无限维向量组成的集合，这些向量的p-范数存在。  
...
$p,q\in[0,+\infty),p^{-1}+q^{-1}=1$  
$\mid(x,y)\mid \leq \mid\mid x\mid\mid_p \mid\mid y\mid\mid_q$  
不等式强调了p=q=2这种情况。  
